import {fetchLocations, fetchWeather, formatTemperature, formatWindSpeed} from "../MetaweatherService";
...
describe("fetchLocations", () => {
  ...
  it("fetches locations", () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith("/km-weather-app/us-central1/search?query=searchQuery");
  });
});
...
describe("fetchLocations", () => {
    ...
    it("fetches locations", () => {
        fetchWeather("id")
        expect(global.fetch).toHaveBeenCalledWith("/km-weather-app/us-central1/location?query=id");
    });
});
